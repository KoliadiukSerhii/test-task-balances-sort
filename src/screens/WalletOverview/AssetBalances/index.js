...
const AssetBalances = ({ balanceData, walletType }) => {
	...
	const openWithdrawModal = (id) => {
		if (verificationData?.verification?.status !== 2) {
			toast.error(verificationRequiredToast.component(t(`wallet.verificationReqWithdraw`)), { id: 'verification_required2', duration: 5000 });
		} else if (Object.values(currency_data[all_currencies_flip[id]]?.networks ?? {})?.[0]?.is_withdraw_on === false) {
			toast.error(t(`wallet.disabledWithdraw`), { id: 'disabled2', duration: 5000 });
		} else if (accountInfo.profile.is_suspended) {
			toast.error(t(`toast.errorCode.367`), { id: 'disabled2', duration: 5000 });
		} else {
			setCurrencyId(id);
	...
	const openDeposit = (id) => {
		if (verificationData?.verification?.status !== 2) {
			toast.error(verificationRequiredToast.component(t(`wallet.verificationReqDeposit`)), { id: 'verification_required1', duration: 5000 });
		} else if (Object.values(currency_data[all_currencies_flip[id]]?.networks ?? {})?.[0]?.is_deposit_on === false) {
			toast.error(t(`wallet.disabledDeposit`), { id: 'disabled1', duration: 5000 });
		} else if (accountInfo.profile.is_suspended) {
			toast.error(t(`toast.errorCode.367`), { id: 'disabled1', duration: 5000 });
		} else {
	...
	/**
	 * Sorts the balances in alphabetical order where the balances are equal to 0 or in descending order by balance in USDT
	 * @param balanceData
	 * @returns {sortedBalances}
	 */
	const defaultSortedBalances = (balanceData) => {
		const zeroBalances = [];
		const nonZeroBalances = [];

		Object.keys(balanceData).map((elem) => {
			if (balanceData[elem].total_available_usdt_num === 0) {
				zeroBalances.push(elem);
			} else {
				nonZeroBalances.push(elem);
			}
		});

		zeroBalances.sort((prev, next) => {
			return prev.localeCompare(next);
		});

		nonZeroBalances.sort((prev, next) => {
			return balanceData[next].total_available_usdt_num - balanceData[prev].total_available_usdt_num;
		});

		return nonZeroBalances.concat(zeroBalances);
	};
	...
	useEffect(() => {
		if (balanceData) {
			const defaultSortedBalancesData = defaultSortedBalances(balanceData);
			setSortedBalanceData(defaultSortedBalancesData);
		}
	}, [balanceData]);
							...
							<div className={styles.table}>
								<div className={styles.row}>
									<div className={styles.col}>
										<div className={`sorting ${checkClassName('name')} `} onClick={() => handleSort('name')}>
											{t(`${page}.sorting.coin`)}
										</div>
									</div>

									<div className={styles.col}>
										<div className={`sorting ${checkClassName('total_balance')}`} onClick={() => handleSort('total_balance')}>
											{t(`${page}.sorting.totalBalance`)}
										</div>
									</div>
									<div className={styles.col}>
										<div className={`sorting ${checkClassName('total_available')}`} onClick={() => handleSort('total_available')}>
											{t(`${page}.sorting.totalAvailable`)}
										</div>
									</div>
									<div className={styles.col}>
										<div className={`sorting ${checkClassName('frozen')}`} onClick={() => handleSort('frozen')}>
